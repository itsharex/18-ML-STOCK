...
- **当前版本**: `1.3.27`（见 `wails.json` 的 `info.productVersion` 与 `frontend/src/Settings.tsx` 的 `const version`）
...
| `frontend/tsconfig.json` | TypeScript 配置：`target: ES2020`、`jsx: react-jsx`、严格模式、`noUnusedLocals: true`、`noUnusedParameters: true`、`noFallthroughCasesInSwitch: true` |
...
```
stockfinlens/
├── main.go                       # Wails 入口（51 行）：embed 前端 dist、股票库、ML 模型
├── app.go                        # App 结构体，所有 Wails 绑定方法（2577 行）
├── storage.go                    # 本地文件存储管理器（JSON 持久化，811 行）
├── csvparser.go                  # 同花顺 CSV/Excel 财报导入解析器（400 行）
├── deps_manager.go               # Python 依赖检测与一键安装管理器（274 行）
...
│   ├── engine.go                 # 18 步分析主入口（RunAnalysisWithAll 等，170 行）
│   ├── steps.go                  # 18 步财务透视具体实现（893 行）
│   ├── evaluator.go              # 评分与评级逻辑（100 分制，A-F 等级）
│   ├── report.go                 # Markdown 报告生成器（14 模块，2653 行）
...
│   ├── data.go                   # 财务数据加载与清洗
│   ├── types.go                  # 核心类型定义（CalcTrace、StepResult、AnalysisReport 等，127 行）
...
│
├── downloader/                   # 数据下载与爬取层（所有网络 I/O，19 个文件）
...
│   ├── concept.go                # 股票概念与风口数据（静态映射）
│   ├── hot_concept.go            # 热门概念板块实时排行（东财 API + 综合打分）
...
│   ├── hot_concept_test.go       # 热门概念 API 与打分算法测试
...
│   ├── src/
│   │   ├── App.tsx               # 主界面组件（~2900 行，自选股、市场热点、分析、报告展示）
│   │   ├── Settings.tsx          # 设置面板（版本号来源之二，289 行）
...
│   │   ├── main.tsx              # 前端入口
│   │   ├── stocks.ts             # 内置股票代码库（8248 行，~600KB）
│   │   ├── api/                  # （预留空目录）
│   │   └── components/           # （预留空目录）
...
│   │   ├── model.py
│   │   └── train.py
│   └── engine_d_risk/            # LightGBM 风险预警模型（engine_d_model.pkl）
│       ├── feature_engineering.py
│       └── train.py
│
├── scripts/                      # Python 数据脚本（打包时必须包含，12 个文件）
...
- 前端状态管理集中在 `App.tsx`（2741 行单文件大组件），新增功能时优先在现有 hooks 体系内扩展，避免引入额外状态管理库
- `frontend/src/api/` 与 `frontend/src/components/` 为预留空目录，当前未使用
...
4. **打标签**: `git tag v1.3.27`, `git push origin v1.3.27`
5. **构建发布包**:
...
7. **版本号递增**: 发布后立刻将两个文件版本号 bump 到下一个未发布版本（如 `1.3.28`）。
